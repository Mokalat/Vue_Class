...
<!--
  ...
  그러나 spa(sing - page - application)에서 클라이언트 즉 자바스크립트가 전체 페이지를 다시 로드하지 않고
  일부분만 동적으로 갱신되는 애플리케이션을 의마하며 서버 라우팅은 웹 애플리케이션에 새로운 페이지를 로드할 때마다
  전체 페이지를 새로고침해야 했으나, spa는 초기 페이지 로드 이후 페이지를 새로고침하지 않고
  필요한 데이터만 받아 동적으로 페이지 일부분을 업데이트하게 된다.
  이러한 spa의 라우팅은 클라이언트 측, 브라우저에서 수행되며 서버에 데이터만 요청하는 방식이다

  vue에서 라우팅을 하기 위해서 vue router를 설치해야한다
  npm install vue-router@next

  vue의 routing
  vue는 url 리소스를 설정할 때 url 속성에 #을 추가하여 리소르를 등록하게 되는데
  이는 vue에서 브라우저의 기본 동작을 방지하기 위함으로 만약 #을 추가하지 않으면
  자바스크립트를 사용하여 내비게이션을 제어하게되며 기존 방식으로 서버 라우팅을 처기하게 된다.
